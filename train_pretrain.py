...
from core.models.layers.mrr_conv2d import AddDropMRRBlockConv2d
...
def main() -> None:
    ...
    if configs.dataset.name in {"tinyimagenet"} or getattr(
        ...
        conv_list1 = [
            i for i in model.modules() if isinstance(i, AddDropMRRBlockConv2d)
        ]
    ...
    try:
        ...
        for epoch in range(int(configs.run.n_epochs)):
            ...
            if configs.noise.weight_noise_std < 1e-6:
                ...
                accv.append(avg_acc)
                lg.info(
                    "Noisy Validation: Average Loss {:.4f} ({:.4f}) Average Accuracy {:.2f}% ({:.4f})".format(
                        avg_loss, std_loss, avg_acc, std_acc
                    )
                )
            saver.save_model(
                model,
