...
def main() -> None:
    ...
    try:
        ...
        acc_vectors_first = []
        acc_vectors_second = []
        delta_step = configs.mapping.probe_interval
        cycles_vector = []
        total_cycles = 0
        total_cali_cycles = 0
        ...
        # probe_num = 0
        probe_cycle = model.probe_cycles()
        inference_cycle = model.cycles(
            [
                1,
        ...
        for step in range(10000):
            ...
            if counter == 0 and (step % delta_step == 0 or step == 10000 - 1):
                lg.info(
                    f"step {step}, T: {global_temperature_scheduler.T, global_temperature_scheduler.T0} K"
                )
                ...
                remap_flag = bool(
                    (
                        (
                            global_temperature_scheduler.T
                            - global_temperature_scheduler._last_T
                        )
                        * global_temperature_scheduler.hotspot_map
                    ).mean()
                    > T_thres
                )
                print(f"temperature flag is? ={remap_flag}")
                if remap_flag is False:
                    remap_flag = model.probe_weight_error() > remap_thres
                    total_cycles += probe_cycle

                print(f"probed weight_noise is = {model.probe_weight_error()}")
                print(f"remap? ={remap_flag}")
                if remap_flag:
                    remapping_cycles = model.remap(
                        input_shape=[
                            1,
            ...
            if step % 50 == 0 or step == 10000 - 1:
                ...
                lg.info(f"step now = {step}")
                lg.info(
                    f"temp now is {global_temperature_scheduler.T}, previous temp is {global_temperature_scheduler._last_T}"
                )
                lg.info(
                    f"total cali + remap cycles now is {total_cali_cycles}({total_cali_cycles/total_cycles:.4f}), total cycles={total_cycles}"
                )

            model.step_noise_scheduler(T=1)
            total_cycles += inference_cycle
            # total_cycles += cycles
            cycles_vector.append(total_cycles)
        # print(f'number of probeing = {probe_num}')
        # print(acc_vectors_noremap)
        # print(acc_vectors_none)
        lg.info(acc_vectors_first)
        # print(acc_vectors_second)

        ensure_dir(
            f"./Experiment/log/{configs.dataset.name}/{configs.model.name}/remap_interv"
        )
