...
FilePath: /L2ight_Robust/script/fmnist/cnn3_mrr8/Sparsity/cnn3_mrr_sparsity.py
"""
import os
import subprocess
...
root = "Experiment/log/fmnist/cnn3_mrr/mainresult"
script = "eval.py"
config_file = "config/fmnist/cnn3_mrr/pm/sparsity.yml"
...
def task_launcher(args):
    ...
    with open(os.path.join(root, f"main_{algo}_cnn3_mrr_{sparsity}_ns-{nsteps}_lr-{lr}_avg-{average_times}_cri-{criterion}_itvl-{validation_interval}_sp-{sparsity_mode}_salience-{salience_mode}_stop-{stop_thres}_pv-{PV_schedule}-{noise_scenario_src}-{noise_scenario_tgt}_td-{TD_schedule}-{hotspot}.log"), "w") as wfid:
        exp = [
...
if __name__ == "__main__":
    ...
    tasks = [
        (   
            ...
            0.00382,
            "./checkpoint/fmnist/cnn3_mrr/pretrain/SparseBP_MRR_CNN_wb-32_ib-32__acc-92.78_epoch-97.pt", # checkpoint
        )
    ...
    tasks = [
        (   
            ...
            'first_grad', #salience_mode
            0.00382,
            "./checkpoint/fmnist/cnn3_mrr/pretrain/SparseBP_MRR_CNN_wb-32_ib-32__acc-92.92_epoch-97.pt", # checkpoint
        )
        for PV in PV_config.values() for TD in TD_config.values()
    ]
    tasks = [
