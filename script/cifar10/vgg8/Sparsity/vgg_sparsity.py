...
root = "Experiment/log/cifar10/vgg8/scan_sparsity"
script = "scan_sparsity.py"
    ...
    for spar_i in sparsity:
        for sparsity_mode in sparsity_modes:
            for salience_mode in salience_modes:
                with open(os.path.join(root, f"sparsity_vgg8_{spar_i}_ns-{nsteps}_lr-{lr}_avg-{average_times}_cri-{criterion}_itvl-{validation_interval}_sp-{sparsity_mode}_salience-{salience_mode}_stop-{stop_thres}.log"), "w") as wfid:
                    exp = [
