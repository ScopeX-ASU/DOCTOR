...
def task_launcher(args):
    ...
    with open(os.path.join(root, f"main_{algo}_vgg8_{sparsity}_ns-{nsteps}_lr-{lr}_avg-{average_times}_cri-{criterion}_itvl-{validation_interval}_sp-{sparsity_mode}_salience-{salience_mode}_stop-{stop_thres}_pv-{PV_schedule}-{noise_scenario_src}-{noise_scenario_tgt}_td-{TD_schedule}-{hotspot}_remapitvl-{remap_interval}.log"), "w") as wfid:
        exp = [
...
if __name__ == "__main__":
    ...
    tasks = [
        (   
            ...
            0.00382,
            "./checkpoint/cifar10/vgg/pretrain/SparseBP_MRR_VGG8_wb-32_ib-32__acc-90.94_epoch-189.pt", # checkpoint
        )
    ...
    tasks = [
        (   
            ...
            'first_grad', #salience_mode
            0.00382,
            "./checkpoint/cifar10/vgg/pretrain/SparseBP_MRR_VGG8_wb-32_ib-32__acc-90.60_epoch-175.pt", # checkpoint
        )
        for PV in PV_config.values() for TD in TD_config.values()
    ]
    ...
    tasks = [
        ...
        for PV in PV_config.values() for TD in TD_config.values() for interval in list(range(8000, 10001, 1000))
